function acov = mlecov1(params, data, varargin)
...
if nargin < 4
    error(message('stats:mlecov:TooFewInputs'));
end

if ~isvector(data)
    error(message('stats:mlecov:InvalidData'));
end

pnames = {'pdf', 'cdf' 'logpdf' 'logsf' 'nloglf' 'censoring' 'frequency' 'options'};
defaults =  {[] [] [] [] [] zeros(size(data)) ones(size(data)) []};
[pdf,cdf,logpdf,logsf,nloglf,cens,freq,options] = ...
...
if ~isempty(nloglf)
    ...
    elseif iscell(nloglf) && isa(nloglf{1},'function_handle')
        ...
        error(message('stats:mlecov:InvalidNloglf'));
    end
    loglikefun = nloglf;
    ...
    try
        ...
        m = message('stats:mlecov:FunEvalError','nloglf',func2str(nloglf));
...
elseif ~isempty(logpdf)
    ...
    elseif iscell(logpdf) && isa(logpdf{1},'function_handle')
        ...
        error(message('stats:mlecov:InvalidLogpdf'));
    end
    if ~isempty(logsf)
        ...
        elseif iscell(logsf) && isa(logsf{1},'function_handle')
            ...
            error(message('stats:mlecov:InvalidLogsf'));
        end
    elseif isempty(logsf) && sum(cens) == 0
        logsfArgs = {};
    else
        error(message('stats:mlecov:LogsfRequired'));
    end
...
elseif ~isempty(pdf)
    ...
    elseif iscell(pdf) && isa(pdf{1},'function_handle')
        ...
        error(message('stats:mlecov:InvalidPdf'));
    end
    if ~isempty(cdf)
        ...
        elseif iscell(cdf) && isa(cdf{1},'function_handle')
            ...
            error(message('stats:mlecov:InvalidCdf'));
        end
    elseif isempty(cdf) && sum(cens) == 0
        cdfArgs = {};
    else
        error(message('stats:mlecov:CdfRequired'));
    end
    ...
    error(message('stats:mlecov:DistFunsRequired'));
end

if isscalar(delta) || isequal(size(delta),size(params))
    deltaparams = delta .* max(abs(params), 1);
else
    error(message('stats:mlecov:InputSizeMismatch'));
end
...
if haveGrad
    ...
    for j=1:nparams
        e(j) = deltaparams(j);
        [~,Gplus]  = feval(loglikefun, params+e, data, cens, freq, addArgs{:});
        [~,Gminus] = feval(loglikefun, params-e, data, cens, freq, addArgs{:});
...
try
    ...
    m = message('stats:mlecov:FunEvalError','logpdf',func2str(logpdf));
    ME2 = MException(m.Identifier,'%s',getString(m));
    throwAsCaller(addCause(ME2,ME));
end
...
if ~isempty(logsf)
    try
        ...
        m = message('stats:mlecov:FunEvalError','logsf',func2str(logsf));
        ME2 = MException(m.Identifier,'%s',getString(m));
        throwAsCaller(addCause(ME2,ME));
    end
...
try
    pdfVals = feval(pdf, data(isUncensored), paramsCell{:}, pdfAddArgs{:});
     ...
     m = message('stats:mlecov:FunEvalError','pdf',func2str(pdf));
     ME2 = MException(m.Identifier,'%s',getString(m));
     throwAsCaller(addCause(ME2,ME));
end
...
if ~isempty(cdf)
    try
        ...
        m = message('stats:mlecov:FunEvalError','cdf',func2str(cdf));
        ME2 = MException(m.Identifier,'%s',getString(m));
        throwAsCaller(addCause(ME2,ME));
    end
